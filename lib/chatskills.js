    ...
    respond: function(input, sessionId, callback) {
        ...
        if (currentSession) {
            ...
            for (var i in Object.keys(app.intents)) {
                ...
                result = ChatSkillsManager.parse(input, utterances, app.intents[key].schema, app.dictionary);
                if (result.isValid) {
    ...
    parse: function(text, utterances, schema, dictionary) {
        ...
        for (var h in utterances) {
            ...
            if (template && template.length > 0) {
                ...
                // Replace date with temporary placeholder for regEx split.
                var dateMatch;
                while (dateMatch = text.match(/(0?[1-9]|1[012])[- /.](0?[1-9]|[12][0-9]|3[01])[- /.](19|20)?[0-9]{2}/)) {
                  var key = '___DATE' + (Object.keys(dates).length + 1);
                  text = text.replace(dateMatch[0], key + ' ');
                  dates[key] = dateMatch[0];
                }

                // Replace times with temporary placeholder for regEx split.
                var timeMatch;
                while (timeMatch = text.match(/(0?[1-9]|1[012])(:[0-5]\d) *[APap][mM]/)) {
                  var key = '___TIME' + (Object.keys(times).length + 1);
                  text = text.replace(timeMatch[0], key + ' ');
                  times[key] = timeMatch[0];
                }
                ...
                if (tokens.length == words.length) {
                    for (var i = 0; i < tokens.length; i++) {
                        ...
                        dateMatch = word.match(/___DATE\d+/);
                        if (dateMatch && dateMatch.length > 0) {
                          word = dates[dateMatch];
                        }

                        timeMatch = word.match(/___TIME\d+/);
                        if (timeMatch && timeMatch.length > 0) {
                          word = times[timeMatch];
                        }
                        ...
                        if (token.toLowerCase() != word.toLowerCase()) {
                            ...
                            if ((tokenParts && tokenParts.length == 3) || (tokenParts2 && tokenParts2.length > 0)) {
                                ...
                                switch (type) {
                                    ...
                                    case 'DATE':
                                    case 'AMAZON.DATE': isValidType = word.match(/(0?[1-9]|1[012])[- /.](0?[1-9]|[12][0-9]|3[01])[- /.](19|20)?[0-9]{2}/); break; // http://stackoverflow.com/a/29648721/2596404
                                    case 'TIME':
                                    case 'AMAZON.TIME': isValidType = word.match(/(0?[1-9]|1[012])(:[0-5]\d) *[APap][mM]/); break; // http://stackoverflow.com/a/8820572/2596404
                                    case 'LITERAL':
